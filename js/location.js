...
function geo_success(position) {
    ...
    if (position.coords.accuracy <= min_accuracy) {
        if (prev_lat != position.coords.latitude || prev_long != position.coords.longitude) {
            ...
            info_string = "Current positon: lat=" + position.coords.latitude.toFixed(6) + ", long=" + position.coords.longitude.toFixed(6) + " (accuracy " + Math.round(position.coords.accuracy, 1) + "m)<br />Altitude: min=" + (min_altitude ? min_altitude : "Not recorded/0") + "m, max=" + (max_altitude ? max_altitude : "Not recorded/0") + "m (accuracy " + Math.round(position.coords.altitudeAccuracy, 1) + "m)<br />last reading taken at: " + current_datetime;
        }
    } else
...
function init_geo() {
    op = document.getElementById("geo-output");
    if (op) {
        if (wpid) {
